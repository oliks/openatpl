...
export async function downloadTestForOffline(savedTestId, subjectId, questionIds, testHref, onProgress) {
  if (!isOfflineSupported()) throw new Error("Offline not supported");

  await navigator.serviceWorker.ready;
  const cache = await caches.open("openatpl-offline-v1");

  // 0. Pre-cache the test runner page so it works offline
  if (testHref) {
    try {
      const pageRes = await fetch(testHref);
      if (pageRes.ok) {
        await cache.put(new Request(testHref, { method: "GET" }), pageRes);
      }
    } catch { /* skip */ }
  }
