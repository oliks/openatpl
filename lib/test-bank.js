...
async function loadQuestionsFromSubjectIndex(payload, sourcePath, selectedFiles) {
  const indexEntries = Array.isArray(payload?.questions) ? payload.questions : [];
  const baseDir = path.dirname(sourcePath);

  // If selectedFiles is provided, only load those specific files
  const toLoad = selectedFiles
    ? indexEntries.filter((e) => selectedFiles.has(asString(asObject(e)?.file)))
    : indexEntries;

  const loaded = [];
  for (const indexEntry of toLoad) {
    ...
    const normalized = normalizeStandardizedQuestionEntry(JSON.parse(raw));
...
async function normalizeQuestionsFromPayload(payload, { sourcePath, testId }) {
  ...
  if (isSubjectIndexPayload(payload)) {
    return loadQuestionsFromSubjectIndex(payload, sourcePath);
  }
...
function resolveSourcePath(sourceFile) {
  return path.isAbsolute(sourceFile) ? sourceFile : path.join(process.cwd(), sourceFile);
}

async function loadTestIndex(config) {
  ...
  const sourcePath = resolveSourcePath(sourceFile);
  ...
  const entries = indexEntries
    .map((e) => {
      ...
      if (!item) return null;
      const qid = normalizeId(item.questionId ?? item.id);
      if (qid == null) return null;
      return {
        id: qid,
        file: asString(item.file),
        correctOption: normalizeOption(item.correctOption),
        hasAttachment: Boolean(item.hasAttachment),
      };
    })
    .filter(Boolean);

  const withAnswers = entries.filter((e) => Boolean(e.correctOption)).length;
  const test = {
    id,
  ...
  const summaries = await Promise.all(
    manifest.map(async (config) => {
      ...
      const withAnswers = questions.filter((q) => {
        const opt = asObject(q) ? normalizeOption(q.correctOption) : null;
        return Boolean(opt);
      }).length;
      const withAttachment = questions.filter((q) => asObject(q)?.hasAttachment).length;
      return {
        id: String(config.id),
        name: asString(config.name) || `Test ${config.id}`,
        subject: asString(config.subject) || "General",
        description: asString(config.description),
        totalQuestions: questions.length,
        answerCoverage: withAnswers,
        withAttachment,
        withoutAttachment: questions.length - withAttachment,
      };
    })
  );
  return summaries
...
export async function getTestById(testId) {
  ...
  return loadTestIndex(config);
}

export async function loadSelectedQuestions(test, selectedEntries) {
  const baseDir = path.dirname(test.sourcePath);
  const loaded = [];
  for (const entry of selectedEntries) {
    if (!entry.file) continue;
    const questionPath = path.resolve(baseDir, entry.file);
    const raw = await readFile(questionPath, "utf-8");
    const normalized = normalizeStandardizedQuestionEntry(JSON.parse(raw));
    if (normalized) loaded.push(normalized);
  }
